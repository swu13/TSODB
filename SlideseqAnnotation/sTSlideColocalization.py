...
Pipeline:
1. Load gene expression and spatial metadata
...
def create_anndata_from_spatial(
    ...
    # Check files
    # =======================
    genefile   = os.path.join(rawpath, "expression.csv")
    region_file   = os.path.join(output, "cell_annotation_spatial.csv")
    for f in [genefile, region_file]:
    ...
    counts = pd.read_csv(genefile, index_col=0)
    counts = counts.T
    ...
    adata.obs["imagerow"] = coords["X"].astype("category")
    adata.obs["imagecol"] = coords["Y"].astype("category")
...
def spcolocalization(
    ...
    # -------- Stromal boundary --------
    adata_SB = adata[adata.obs["region"] == "Interface"].copy()
    print(f"Interface cell number: {adata_SB.n_obs}")
    if adata_SB.n_obs > 0:
        df1 = run_enrichment(adata_SB,"Interface")
        df = pd.concat([df, df1])
        
    # -------- Stromal core --------
    adata_SB = adata[adata.obs["region"] == "Stroma"].copy()
    print(f"Stroma cell number: {adata_SB.n_obs}")
    if adata_SB.n_obs > 0:
        df1 = run_enrichment(adata_SB,"Stroma")
        df = pd.concat([df, df1])
