...
#' @export
#'
SpaCET.deconvolution.malignant.revised <- function(SpaCET_obj, Malignant="Malignant", malignantCutoff=0.7, coreNo=6)
{
  ...
  pcs <- getPcs(matnorm[matnorm.info$ods,],nGenes=length(matnorm.info$ods),nPcs=min(30,length(matnorm.info$ods)-1),verbose=FALSE)
...
inferMal_cor <- function(st.matrix.data, cancerType, signatureType)
{
  ...
  if(ncol(st.matrix.data.diff) < 20000)
  {
    ...
    matnorm <- log10(matnorm.info$mat+1)
    pcs <- getPcs(matnorm[matnorm.info$ods,],nGenes=length(matnorm.info$ods),nPcs=min(length(matnorm.info$ods)-1,30),verbose=FALSE)
